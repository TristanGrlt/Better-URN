...
/**
 * Filters out events that are individually hidden or belong to a course title marked as hidden.
 */
fun filterVisibleEvents(
    events: List<EdtEvent>,
    hiddenEventIds: Set<String>,
    hiddenCourseTitles: Set<String>
): List<EdtEvent> {
    if (hiddenEventIds.isEmpty() && hiddenCourseTitles.isEmpty()) return events
    return events.filter { event ->
        if (event.id in hiddenEventIds) return@filter false
        val courseTitle = event.title.trim()
        if (courseTitle in hiddenCourseTitles) return@filter false
        true
    }
}

/**
 * Extracts a sorted list of unique course titles from events belonging to [timetableId] (or all events if null).
 */
fun extractUniqueCourseTitles(
    events: List<EdtEvent>,
...
/**
 * Generates an immutable signature for a timetable event based on its cleaned title and start time.
 * This signature remains stable even if the ICS file is regenerated by the administration.
 */
fun generateEventSignature(title: String, startMs: Long): String {
    return "${cleanSubjectName(title).hashCode()}_$startMs"
}
